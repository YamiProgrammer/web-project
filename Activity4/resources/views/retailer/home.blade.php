<x-retailer-layout>
    <div class="container p-6">
        <h1 class="pb-6">Reseller Viewing Retailer Products</h1>
                    ...
                    <div class="relative mx-4 mt-4 h-auto overflow-hidden rounded-xl bg-white bg-clip-border text-gray-700">
                        ...
                        <div class="p-6">
                            <div class="mb-2 items-center">
                                <p class="block font-sans text-base font-medium leading-relaxed text-blue-gray-900 antialiased">{{ $product->productName }}</p>
                                @if ($product->retailerProduct)
                                    <p class="">Price: {{ $product->retailerProduct->retailer_price }}₱</p>
                                @endif
                            </div>
                            ...
                            <form action="{{ route('reseller.orders.store') }}" method="POST">
                                ...
                                <div class="form-group">
                                    <input type="number" name="quantity" class="form-control block w-full select-none rounded-lg bg-blue-gray-900/10 py-3 px-6 text-center align-middle font-sans text-xs font-bold uppercase text-blue-gray-900 transition-all hover:scale-105" placeholder="Quantity" min="1" max="{{ $product->AvailableStocks }}" required>
                                </div>
