...
<div class="container mx-auto py-8">
        ...
        <div class="flex justify-between items-center pt-2 relative mx-auto mb-6 text-gray-600">
            <form action="{{ route('retailer.manage-product') }}" method="GET" class="flex space-x-2">
        ...
        <div id="createForm" class="fixed inset-0 items-center justify-center bg-black bg-opacity-50 z-50 hidden">
            <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('retailer.manage-product.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                            ...
                            <tr class="bg-white sm:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
                                ...
                                <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
                                    @if ($product->retailerProduct)
                                        <p class="mb-2">{{ $product->retailerProduct->retailer_price }}₱</p>
                                    @endif
                                </td>
                                <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
                                ...
                                <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
                                    <a href="{{ route('retailer.manage-product', ['category' => $product->category->categoryID]) }}" class="text-blue-500 hover:underline">{{ $product->category->categoryName }}</a>
                                </td>
                                <td class="w-full lg:w-auto p-3 flex space-x-2 relative lg:static">
                                    <a href="{{ route('retailer.manage-product.edit', ['productID' => $product->productID]) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded">Edit</a>
